          ...
          {products.map((product, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative overflow-hidden rounded-3xl bg-surface border border-white/5 hover:border-accent/40 transition-colors duration-500 shadow-lg hover:shadow-[0_15px_40px_rgba(56,152,212,0.15)] h-[450px] md:col-span-1 ${gridSpan} transform-gpu`}
              >
                ...
                <div className="absolute inset-0 z-20 p-8 flex flex-col justify-end">
                  ...
                  <div className="overflow-hidden h-8">
                    <a href={`/brand/${product.title.toLowerCase().replace(/\s+/g, '-')}`} className="inline-flex items-center gap-2 text-sm text-accent font-semibold tracking-wide hover:text-white transition-colors uppercase transform-gpu translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 duration-500">
