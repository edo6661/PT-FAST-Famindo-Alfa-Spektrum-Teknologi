      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10 max-w-7xl">
        <div className="mb-16">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <span className="text-accent font-bold tracking-widest uppercase text-xs">{t('categoryDetail.badge')}</span>
              <h1 className="text-4xl md:text-5xl font-bold text-white tracking-tight">{t(`catalog.categories.${category.id}.name`, { defaultValue: category.name })}</h1>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {categoryProducts.length === 0 && (
            <div className="col-span-full text-center py-12 border border-white/10 rounded-3xl bg-surface/30">
              <p className="text-foreground-muted">{t('categoryDetail.empty')}</p>
            </div>
          )}
        </div>
      </div>
